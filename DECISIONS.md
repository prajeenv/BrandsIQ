...
**Context.** The Add Review form forced both Platform and Review Text. The founder observed that many real customers (especially on Google) leave only a star rating with no written comment, and a business that wants to respond to such a review could not even add it. The requiredness is inverted: rating becomes required, review text optional. Rating is the cleaner anchor because the AI response generator always has something to respond to.

#### Decision 109: Rating required + review text optional, with a no-text AI generation path; templates stay pure, migration is additive-nullable.

- **Decision (validation):** `createReviewSchema` makes `reviewText` optional (`z.string().max(4000).optional()`) and `rating` required (`z.number({ message: "Please select a rating" }).min(1, ...).max(5, ...)`). `updateReviewSchema` is unchanged (PATCH semantics already had both optional). The friendly message covers both the missing/invalid case and the range case so the form shows one consistent hint.
- **Decision (DB):** `Review.reviewText` becomes `String? @db.Text` via additive migration `20260604120000_review_text_optional` (`ALTER COLUMN ... DROP NOT NULL`). No backfill, no contract half — this is a loosening, not a tightening. `rating` stays `Int?` at the DB level so edit/PATCH can still clear it; the create schema is what enforces presence.
- **Decision (route, `POST /api/reviews`):** the handler normalizes empty/whitespace `reviewText` → `null` (load-bearing — without it blank text would reach DeepSeek), then for a star-only review: (a) skips the 5-minute text-based duplicate check entirely (text dedupe catches accidental re-submits of the same *comment*; star-only reviews have nothing to duplicate, and bulk-adding several star-only reviews is a real workflow), (b) skips language detection (defaults to English), (c) skips sentiment analysis and spends no sentiment credit, (d) omits the `sentimentWarning` (it now fires only when sentiment was skipped because credits ran out — a star-only skip is expected, not a credit problem, which also stops a spurious "sentiment skipped" banner on every star-only add).
- **Decision (AI, the core):** `GenerateResponseParams.reviewText` becomes `string | null`. `generateReviewResponse` computes `hasReviewText` and threads it into both prompt builders. `buildUserPrompt` emits "The reviewer left a N-star rating with no written comment." instead of an empty `wrapUserContent("Customer review", "")` block (the empty block is what invites hallucination). `buildSystemPrompt` injects a no-text override block AFTER the structure template (so it overrides the "Specificity is required" instruction) and BEFORE the reinforcement tail (so style/security rules still win). The block tells the model not to reference, quote, or invent any specific incident/detail/dish/staff/occasion, and to respond to the sentiment of the rating itself. **The structure templates in `structure-templates.ts` are untouched** — `selectStructureTemplate({rating, sentiment: null})` already routes by rating alone (1-2★ negative, 3★ mixed, 4-5★ positive), so star-only reviews route correctly with zero template changes.
- **Decision (generate/regenerate routes):** both pass `review.reviewText ?? ""` to `logIfInjectionAttempt` (its detector takes a `string` and would throw on null; an empty string yields no matches, which is correct). The `generateReviewResponse` calls type-check unchanged once the param is nullable.
- **Decision (form):** Platform label drops the misleading `*` (it is a closed dropdown defaulting to Google and can never be empty). Review Text label → "Review Text (optional)". Rating label → "Rating *" with an inline error. The star widget is wired via RHF **`Controller`** rather than `setValue`/`watch` — `setValue` on an unregistered field did not surface `errors.rating` on empty submit (the flagged risk; confirmed by a failing test), and a registered hidden `<input>` injected an empty-string/`NaN` type conflict. `Controller` is the idiomatic RHF way to put a custom widget into validation and keeps the field number-typed. The star picker no longer clears on re-click (rating cannot be blanked on create); re-picking a different star changes it.
- **Decision (null-text display):** `ReviewCard` and the review detail page typed `reviewText: string`; both called `needsExpansion`/`truncate*` which throw on null. Both now render a muted "No written comment" placeholder for star-only reviews and guard the expansion control. The two credit/sentiment usage routes already used optional chaining and needed no change.
- **Alternatives considered:** (a) keep text required, make rating optional — rejected; doesn't solve the star-only case and leaves the AI with zero signal when both are absent. (b) `z.coerce.number()` for rating — rejected; it makes the Zod input type `unknown`, breaking `useForm<CreateReviewInput>` typing. (c) a dedicated fourth "rating-only" structure template — rejected; it would duplicate the positive/mixed/negative tone handling. The conditional prompt-injection approach keeps the templates pure (consistent with the module's design).
- **GDPR posture:** unchanged. `reviewText` is the same data, now nullable; cascade-deletes with the review.
- **Risk:** Low ✅. Additive migration, reversible, no data touched (existing rows all have text). The only behavioural change is that star-only reviews are now addable and respond appropriately.

---

## Walk-in "Start free beta" → /auth/get-started gateway

**Context.** The walk-in page's two "Start free beta" buttons linked straight to `/auth/signup?utm_source=walkin`, dropping a just-met visitor into a bare account-creation form. But BrandsIQ is in closed beta (invite-only), so a direct signup only ever lands them on the Free tier — the visitor has no way to *request* beta access from that path. The founder wanted "Start free beta" to land on a page that offers both: request beta access (the founder follows up) and a regular-signup fallback. This is built almost entirely by reusing existing machinery.

#### Decision 110: New `/auth/get-started` gateway, phase-aware, reusing `FounderInquiryForm`; new `signup_gateway` source; walk-in CTA re-pointed.

- **Decision (new page):** `src/app/(auth)/auth/get-started/page.tsx` — a server component under the `(auth)` group (inherits the centered-card chrome). It mirrors the expired-link page layout: a `Card` with a welcoming heading ("Start your free beta", `Rocket` icon — deliberately NOT the expired page's amber `AlertCircle`), an embedded `<FounderInquiryForm type="beta_request" source="signup_gateway" />` (no prefill → anonymous visitor sees Name / Email* / Business name / "Tell us about your business *", exactly what the founder asked for), and a footer with "Continue with regular signup →" + "Already have an account? Sign in".
- **Decision (phase-awareness):** the page reads the server-only `getCurrentPhase()`. Under `phase_2` (commercial launch, no closed beta) there is no beta to request, so it `redirect()`s to `/auth/signup` preserving `utm_source`. Under `phase_1` (default) it renders the request-beta + signup layout. This matches how `pricing-client` gates its beta CTA on `isPhase1`.
- **Decision (utm threading):** the gateway extracts `utm_source` from the Next 14 synchronous `searchParams` prop (normalizing `string | string[] | undefined` to the first value) and threads it onto BOTH the `phase_2` redirect target AND the "Continue with regular signup" link via a shared `signupHref(utm)` helper. So `walkin → /auth/get-started?utm_source=walkin → /auth/signup?utm_source=walkin`, and `SignupForm`'s existing client-side `utm_source` read keeps working — no prop plumbing needed.
- **Decision (new source):** added `signup_gateway` to `FOUNDER_INQUIRY_SOURCES` so inquiries from this surface are distinguishable in the admin view + PostHog. The Zod schema (`z.enum(FOUNDER_INQUIRY_SOURCES)`) picks it up automatically. The one manual edit needed: `posthog-events.ts:trackFounderInquirySubmitted` has a *hardcoded* `source` union (duplicated, not derived from the enum) — `| "signup_gateway"` was added there too, or type-check would fail. No exhaustive switch over the source enum exists anywhere else (admin + email render it as a raw string).
- **Decision (walk-in only):** only the walk-in page's `SIGNUP_HREF` constant changed (covers both CTA sites). The marketing landing page (`src/app/page.tsx`) still links straight to `/auth/signup` — it is a different funnel and was left untouched per the founder.
- **Alternatives considered:** (a) add the beta-request block to the signup page itself — rejected, mixes two flows and complicates `SignupForm` (which reads `?b=` invite codes). (b) reuse the expired-link page — rejected, its "this invite has expired" copy is wrong for someone clicking "Start free beta". (c) skip phase-awareness — rejected, would leave a "closed beta" request form live after commercial launch.
...
- New page `src/app/(auth)/auth/get-started/page.tsx` — server component, mirrors the expired-link layout (Card + `FounderInquiryForm type=beta_request source=signup_gateway` + "Continue with regular signup →" + sign-in link). Phase-aware: `phase_2` redirects to `/auth/signup` preserving `utm_source`.
- `src/lib/constants.ts`: added `signup_gateway` to `FOUNDER_INQUIRY_SOURCES`. `src/lib/posthog-events.ts`: added `| "signup_gateway"` to the hardcoded `trackFounderInquirySubmitted` source union (the one type-check landmine).
...
- Tests: new `tests/unit/app/auth/get-started-page.test.tsx` (phase_1 renders form + utm-threaded signup link, array-param handling, phase_2 redirect with/without utm); `constants.test.ts` gains a `FOUNDER_INQUIRY_SOURCES` block; `posthog-events.test.ts` gains a `signup_gateway` case.
- Docs: DECISIONS.md (this entry + Decision 110), PROGRESS.md, MVP.md §13.4 (fifth surface note), WALKIN_LANDING_PAGE.md (CTA targets + attribution chain).
- Verification: `npx tsc --noEmit` clean, `npm run lint` clean, affected unit tests green.

**June 4, 2026** — Add Review: rating required, review text optional (Decision 109)
- Branch `feat/star-only-reviews` (off main, after Decision 108 merged). Inverts the Add Review requiredness so a star-only review (rating, no comment) is addable and gets a rating-appropriate AI response. Supports the common Google "5 stars, no comment" case the founder observed.
- `createReviewSchema`: `reviewText` optional, `rating` required with a "Please select a rating" message. `prisma/schema.prisma`: `Review.reviewText` → `String?` + additive migration `20260604120000_review_text_optional` (DROP NOT NULL, no backfill).
- `POST /api/reviews`: normalizes empty text → null; skips dedupe, language detection, and sentiment (no credit spent) for star-only reviews; `sentimentWarning` now fires only on a genuine no-credits skip.
- `claude.ts`: `hasReviewText` threads into both prompt builders; no-text user-prompt line + system-prompt override block (placed after the structure template, before the reinforcement tail) so the model responds to the rating without inventing specifics. Structure templates unchanged — rating-only routing already worked. `generate`/`regenerate` routes pass `review.reviewText ?? ""` to the injection logger.
- `ReviewForm`: Platform `*` dropped, Review Text "(optional)", Rating "*" with error; rating wired via RHF `Controller` (the reliable way to validate a custom widget); no clear-on-reclick. `ReviewCard` + review detail render a "No written comment" placeholder for null-text reviews.
- Tests: `createReviewSchema` block rewritten (star-only accepted, missing/null rating rejected) + rating added to the date/name/language/platform cases; 3 new route tests (star-only create with null text + no warning, dedupe skipped, sentiment skipped even with credits); form tests (labels flipped, blocked-without-rating, star-only submit); claude.ts no-text prompt tests; ReviewCard placeholder test.
- Docs updated: CORE_SPECS.md (validation rules + POST shape), IMPLEMENTATION_GUIDE.md (Review Addition Flow + star-only note), this entry + Decision 109, PROGRESS.md.
